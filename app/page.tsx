  ...
  return (
    <section id="timeline" className="mt-7 scroll-mt-20 border border-[#bfcdd3] bg-white shadow-[0_10px_30px_rgba(31,79,118,.05)] sm:rounded-[10px]" aria-labelledby="timeline-title">
      ...
      <div className="md:hidden">
        {phaseGroups.map((phase) => { const open = openPhases.includes(phase.name); const completedCount = phase.rows.filter((row) => row.actual?.status === 'completed').length; return <section key={phase.name} className="border-b border-[#e1e6e8] last:border-0"><button onClick={() => togglePhase(phase.name)} className="flex min-h-14 w-full items-center justify-between gap-3 bg-[#edf2f4] px-4 py-3 text-left" aria-expanded={open}><span className="min-w-0"><strong className="block text-[14px] font-semibold leading-5">{phase.name}</strong><span className="mt-1 block font-data text-[10px] text-[#75858c]">{shortRange(phase.range)} · {phase.rows.length} 个节点{completedCount ? ` · ${completedCount} 已完成` : ''}</span></span><ChevronDown className={`size-4 shrink-0 text-[#72858e] transition ${open ? 'rotate-180' : ''}`} /></button>{open && <div className="space-y-2 p-3 sm:p-4">{phase.rows.map((row) => { const key = rowKey(row); const selected = key === selectedKey; const state = stateFor(row); return <div data-timeline-row={rowDomToken(row)} key={`${phase.name}:${row.name}`}><button onClick={() => openTimelineRow(row)} aria-expanded={selected} className={`min-h-16 w-full border-l-2 px-3 py-2.5 text-left transition active:bg-[#e7eef2] ${selected ? 'border-[#1f4f76] bg-[#edf3f6]' : 'border-[#8fa5af] bg-[#f7f9fa]'}`}><span className="flex min-w-0 items-start justify-between gap-2"><span className="flex min-w-0 flex-wrap items-center gap-1.5"><strong className="text-[16px] font-semibold leading-6">{row.name}</strong><i className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] not-italic ${state.tone}`}>{state.shortLabel}</i></span><ChevronDown className={`mt-1 size-3.5 shrink-0 text-[#718791] transition ${selected ? 'rotate-180' : ''}`} /></span><span className="mt-1.5 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-[10px] text-[#77868c]"><span className="flex items-center gap-1"><Link2 className="size-3" />{row.linkedItemIds.length} 个关联事项 · 点击展开</span><span className="font-data text-[#5f737e]">{row.ranges.map(shortRange).join(' · ')}</span></span></button>{selected && rowDetail(row, true)}</div>; })}</div>}</section>; })}
      </div>
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-6 sm:px-7 lg:px-10 lg:py-8">
        <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-[15px] leading-7 text-[#68757b] sm:text-sm">时间总表是主要入口。选择计划节点即可查看关联采购、型号、安装条件与下一步；实际完成情况只在收到现场确认后更新。</p>
          </div>
